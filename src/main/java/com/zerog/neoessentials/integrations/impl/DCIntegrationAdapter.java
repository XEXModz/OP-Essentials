...
public class DCIntegrationAdapter implements ChatIntegrationAdapter {
   ...
   public boolean initialize() {
      this.dcIntegrationLoaded = ModList.get().isLoaded("dcintegration");
      if (!this.dcIntegrationLoaded) {
         ...
         try {
            ...
            if (this.tryInitDiscordIntegration()) {
               LOGGER.info("DCIntegration DiscordIntegration messaging API initialised");
               return true;
            } else if (this.tryInitChannelHandler()) {
               LOGGER.info("DCIntegration ChannelHandler messaging API initialised");
               return true;
            } else {
               LOGGER.warn("DCIntegration detected but no supported messaging API found. Events will be logged only. Ensure DCIntegration is up to date.");
               this.dcIntegrationLoaded = true;
               return true;
            }
         } catch (Exception var2) {
            LOGGER.error("Failed to initialize DCIntegration integration: {}", var2.getMessage(), var2);
            return false;
         }
      }
   }
   ...
   private boolean tryInitDiscordIntegration() {
      try {
         Class<?> diClass = Class.forName("de.erdbeerbaerlp.dcintegration.common.DiscordIntegration");
         ...
         LOGGER.debug("DiscordIntegration.sendMessage not found: {}", var4.getMessage());
         return false;
      }
   }
   ...
   private boolean tryInitChannelHandler() {
      try {
         Class<?> utilClass = Class.forName("de.erdbeerbaerlp.dcintegration.common.util.DiscordMessage");
         Method method = utilClass.getMethod("sendToChannel", String.class, String.class);
         this.messagingInstance = null;
         this.sendMessageMethod = method;
         return true;
      } catch (Exception var3) {
         LOGGER.debug("DiscordMessage.sendToChannel not found: {}", var3.getMessage());
         return false;
      }
   }
   ...
   public boolean isEnabled() {
      return this.dcIntegrationLoaded;
   }
   ...
   private void sendToDiscord(String channel, String message) {
      if (this.sendMessageMethod != null) {
         try {
            ...
            LOGGER.warn("DCIntegration sendMessage failed for channel '{}': {}", channel, var4.getMessage());
         }
      } else {
