...
public class VanishManager {
      ...
      if (group == null) {
         ...
         switch (var6) {
            case "owner":
               return 0;
            case "admin":
               // ATMOP: the 'admin' group wears the &4[Owner] tag = the server owner
               // (Spock). Treat as top-tier GODFATHER priority 0 so it's hidden from
               // everyone and outranks all other staff for see-vanished.
               return 0;
            case "co-owner":
               // ATMOP: the 'co-owner' group wears &c[Co-Owner]. Was previously
               // unmatched -> fell through to 10 (a normal player) so vanished
               // co-owners LEAKED (visible on radar/tab). Now a proper staff tier.
               return 1;
            case "mod":
            case "moderator":
               return 2;
   ...
   // ATMOP 1.1.12 (Spock): STAFF ALWAYS SEE VANISHED STAFF — mutually, with no
   // toggle and no rank ladder between them. Staff = group priority <= 3
   // (admin=&4[Owner] 0, co-owner=&c[Co-Owner] 1, moderator=&2[Admin] 2,
   // helper 3). VIP (5) and every ladder rank / default (10) stay normal
   // viewers and can NEVER see vanished staff.
   private boolean isStaffViewer(UUID playerId) {
      return this.getPlayerPriority(playerId) <= 3;
   }
   ...
   private void broadcastFakePresence(MinecraftServer server, ServerPlayer target, boolean join) {
      try {
         ...
         for (ServerPlayer viewer : server.getPlayerList().getPlayers()) {
            // ATMOP 1.1.12: staff still SEE the vanished player, so don't send
            // them the fake leave/join line — only real players get fooled.
            if (!viewer.getUUID().equals(target.getUUID())
                  && !this.canPlayerSeeVanished(viewer.getUUID())
                  && !this.isStaffViewer(viewer.getUUID())) {
   ...
   public void onPlayerJoin(ServerPlayer player) {
      ...
      // ATMOP 1.1.12: staff joiners skip the re-hide entirely — the vanilla
      // player-list sync already showed them the vanished staff, and staff
      // must always see staff. Only real players get re-hidden packets.
      if (server != null && ConfigManager.isHideFromTabListEnabled() && !this.isStaffViewer(playerId)) {
         ...
         if (server != null) {
            ...
            for (ServerPlayer otherPlayer : server.getPlayerList().getPlayers()) {
               if (otherPlayer != vanishedPlayer) {
                  // ATMOP 1.1.12: staff always see vanished staff — never hide
                  // from a staff viewer, regardless of rank between them.
                  if (this.isStaffViewer(otherPlayer.getUUID())) {
                     continue;
                  }
   ...
   private void hidePlayerFromSpecific(ServerPlayer vanishedPlayer, ServerPlayer observer) {
      try {
         ...
         // BUGFIX 1.1.10: ALSO despawn the entity on the observer's client. Tab
         // removal alone leaves the entity tracked client-side: minimap/radar
         // mods still show a dot at the vanished player's position and their
         // sounds still play. With the entity removed, later tracker re-adds
         // are dropped by modern clients (no player-info) — so this sticks.
         observer.connection.send(new net.minecraft.network.protocol.game.ClientboundRemoveEntitiesPacket(vanishedPlayer.getId()));
